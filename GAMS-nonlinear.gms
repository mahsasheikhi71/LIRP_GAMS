...
equations
totalobj,Obj1,obj2,obj3,Cn1,Cn2,Cn3,Cn4,Cn5,Cn6,Cn7,Cn8,Cn9,Cn10,Cn11,Cn12,Cn13,Cn14,Cn15,Cn16,Cn17,Cn18,Cn181,Cn182,Cn19,Cn20,Cn21,Cn22,Cn23,Cn24,Cn241,Cn25,Cn26,Cn27,Cn28,Cna1,Cna2,Cna3,Cna4,Cna5,Cna6,Cna7;

totalobj..total=e=f3;

Obj1..Profit=e=sum(j,sum(p,sum(q$(ord(q) ge 2),sum(k,sum(t,SC(p,q)*II(j,k,p,q,t))))))+sum(j,sum(k,sum(p,sum(t,O(j,k,p,t)*SC(p,"1")))))-(sum(t,sum(j,sum(p,sum(q,hc(j,p)*S(j,p,q,t)))))+sum(j,F(j)*A(j))+sum((r,t,v,h,kp),d(h,kp)*tc(h,kp)*X(h,kp,v,r,t))+sum((t,j,i,p),orr(i,j,p,t)*ps(p))+sum((t,v),vc*NV(v,t))+sum(i,sum(j,sum(t,Z(i,j,t)*os(i,j)))));
obj2..f2=e=sum(h,sum(kp,sum(v,(sum(r,sum(t,exp(0.1929169*(u-80)-0.0024244*power((u-80),2)*x(h,kp,v,r,t))))))));
Obj3..f3=e=sum(h,sum(kp,sum(v,(sum(r,sum(t,(1576-17.6*u+0.00117*power(u,3)+36076/power(u,2))*x(h,kp,v,r,t)*d(h,kp)))))));
...
model Sheikhi/All/;
option reslim=6000;
solve Sheikhi using MINLP minimizing total;
